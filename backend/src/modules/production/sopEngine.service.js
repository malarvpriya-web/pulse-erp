...
export async function runSOP({ companyId, horizonDays = 168, bucketDays = 28 }) {
  ...
  const fc = (await pool.query(`SELECT item_id,product_name,forecast_date,(COALESCE(quantity,0)-COALESCE(consumed_qty,0)) qty FROM demand_forecasts WHERE ${scopeExpr(companyId)} AND (COALESCE(quantity,0)-COALESCE(consumed_qty,0))>0 AND (run_id IS NULL OR COALESCE(status,'draft')='approved')`)).rows;
