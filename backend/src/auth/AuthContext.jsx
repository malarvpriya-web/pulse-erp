...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(async (email, password) => {
    const loginRes = await api.post('/auth/dev-login', { email, password });
  ...
  const can     = useCallback((perm) =>
    role === 'super_admin' || permissions.some(p => p.module === perm && p.can_view),
  [role, permissions]);
  ...
  return (
    <AuthContext.Provider value={{
      ...
      isLoggedIn: !!token,
      initializing,
      login, logout, hasPermission, getVisibleModules, hasRole, can,
    }}>
      {children}
