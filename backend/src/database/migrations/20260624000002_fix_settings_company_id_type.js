export async function up(knex) {
  ...
  const safe = async (sql) => {
    const name = `sp_fix_settings_cid_${sp++}`;
    ...
    try {
      ...
      if (!/already exists|does not exist|does not exist/.test(err.message || '')) throw err;
    }
  };

  // Fix crm_settings if it was created with UUID company_id (type mismatch with companies.id INTEGER)
  await safe(`
    DO $$
    ...
    BEGIN
      ...
      WHERE table_name = 'crm_settings' AND column_name = 'company_id';
