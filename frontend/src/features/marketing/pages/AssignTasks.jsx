...
const BLANK = { campaign_id: '', title: '', assigned_to: '', due_date: '', priority: 'medium', description: '' };
...
export default function AssignTasks() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [tasksRes, campsRes, empsRes] = await Promise.allSettled([
        api.get('/marketing/tasks', { params }),
        api.get('/marketing/campaigns'),
        api.get('/employees'),
      ]);
      setRows(tasksRes.status === 'fulfilled' && Array.isArray(tasksRes.value?.data) ? tasksRes.value.data : []);
  ...
  const handleComplete = async (id) => {
    try { await api.patch(`/marketing/tasks/${id}/complete`); load(); } catch { /* silent */ }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post('/marketing/tasks', form);
      setShowForm(false);
      load();
    } catch { /* silent */ }
        ...
        <>
          ...
          <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: 440, background: 'var(--color-background-primary)', border: '0.5px solid var(--color-border-tertiary)', zIndex: 50, padding: 24, overflowY: 'auto', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ margin: 0, color: 'var(--color-text-primary)', fontSize: 17 }}>Assign Task</h3>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--color-text-secondary)', marginBottom: 4 }}>Task Title *</label>
        ...
        {loading ? (
          [1,2,3].map(i => (
            <div key={i} style={{ display: 'flex', gap: 12, padding: '12px 16px', borderBottom: '0.5px solid var(--color-border-tertiary)' }}>
              {[160, 120, 110, 90, 70, 80, 60].map((w, j) => (
          ...
          <div style={{ padding: 24 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 24px', textAlign: 'center', background: 'var(--color-background-secondary)', borderRadius: 10, border: '0.5px solid var(--color-border-tertiary)' }}>
              <ClipboardList size={36} style={{ color: 'var(--color-text-secondary)', marginBottom: 12 }} />
              <p style={{ fontWeight: 500, fontSize: 15, color: 'var(--color-text-primary)', margin: '0 0 4px' }}>No tasks found</p>
              <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: '0 0 16px' }}>Assign your first task to a campaign team member.</p>
              <button onClick={openNew} style={{ padding: '8px 20px', background: '#6B3FDB', color: '#fff', border: 'none', borderRadius: 7, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>Assign Task</button>
            </div>
          </div>
        ) : (
                ...
                {paged.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id ?? i} style={{ borderBottom: '0.5px solid var(--color-border-tertiary)', background: overdue ? 'rgba(220,38,38,0.04)' : 'transparent' }}>
                      <td style={{ padding: '10px 14px', fontWeight: 600, color: overdue ? '#dc2626' : 'var(--color-text-primary)', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {r.title || '—'}
                      </td>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        {r.priority
                          ? <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 11, fontWeight: 600, background: pc.bg, color: pc.color }}>{r.priority}</span>
                          : '—'}
                      </td>
                      <td style={{ padding: '10px 14px' }}>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        {r.status !== 'completed' && (
                          <button onClick={() => handleComplete(r.id)} title="Mark complete"
