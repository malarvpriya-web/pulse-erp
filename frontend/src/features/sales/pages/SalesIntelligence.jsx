...
export default function SalesIntelligence() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f9fc', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: 'linear-gradient(135deg, #16a34a 0%, #14532d 100%)', padding: '18px 28px 0' }}>
        <div style={{ color: '#fff', fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', opacity: 0.75, marginBottom: 4 }}>
          Sales
        </div>
        <h1 style={{ margin: '0 0 2px', fontSize: 22, fontWeight: 700, color: '#fff' }}>Sales Intelligence</h1>
        <p style={{ margin: '0 0 16px', fontSize: 13, color: 'rgba(255,255,255,0.75)' }}>
          Conversion analytics, sales funnel and forecasting — all your sales insights in one view
        </p>
      ...
      <div>
        {active === 'conversion' && <SalesConversionAnalytics />}
        {active === 'funnel'     && <SalesFunnel />}
        {active === 'forecasts'  && <SalesForecasts />}
      </div>
    </div>
  );
}
