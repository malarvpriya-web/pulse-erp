  ...
  return (
    <PageShell dock={
      <PageHero
        ...
        subtitle={user?.customer_name ? `Welcome, ${user.customer_name}` : 'Your equipment, tickets, documents and AMC in one place'}
        actions={<>
          <button className="plh-cta" onClick={() => setShowRaiseTicket(true)}>
            <Plus size={14} /> Raise Ticket
          </button>
          <button className="plh-cta plh-cta--ghost" onClick={logout}>Sign Out</button>
        </>}
      />
    }>
      {/* The tab strip and all four tab bodies used to be the `actions` payload
          above — 187 lines of workspace inside a prop meant for buttons, which
          renders into `.plh-hero-r` at the right-hand end of the hero row. Same
          hero-kit artefact as Vendor360 (§132) and Project360 (§139); this page
          additionally kept its own purple identity bar below the hero, so it
          rendered two stacked gradient bands. The hero now carries the portal
          identity and its two real actions; the workspace is the page body. */}
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16 }}>
        {[['equipment','My Equipment','📦'],['tickets','Support Tickets','🎫'],['documents','Documents','📄'],['amc','AMC & Service','🔧']].map(([t,l,ic]) => (
          <button className="plh-cta" key={t} onClick={() => setTab(t)}
            style={tab === t ? { background:'#6B3FDB', color:'#fff' } : undefined}>
            {ic} {l}
          </button>
        ))}
      </div>
